<script lang="ts">
	...
	let ethernetSettings: EthernetSettings = $state();
	...
	async function getAPSettings() {
		try {
			...
			ethernetSettings = await response.json();
		} catch (error) {
			console.error('Error:', error);
		}
	...
	async function postAPSettings(data: EthernetSettings) {
		try {
			const response = await fetch('/rest/ethernetSettings', {
				method: 'POST',
				headers: {
	...
	function handleSubmitAP() {
		...
		// Validate gateway IP
		if (!regexExp.test(ethernetSettings.gateway_ip)) {
			valid = false;
		...
		if (!regexExp.test(ethernetSettings.subnet_mask)) {
			valid = false;
		...
		// Validate local IP
		if (!regexExp.test(ethernetSettings.local_ip)) {
			valid = false;
		...
		if (valid) {
			postAPSettings(ethernetSettings);
		}
	}
		...
		{:then nothing}
			...
			>
				<div class="rounded-box bg-base-100 flex items-center space-x-3 px-4 py-2">
					<div
						class="mask mask-hexagon h-auto w-10 {ethernetStatusDescription[ethernetStatus.status].bg_color}"
					>
						<AP class="h-auto w-full scale-75 {ethernetStatusDescription[ethernetStatus.status].text_color}" />
					</div>
					<div>
						<div class="font-bold">Status</div>
						<div class="text-sm opacity-75">
							{ethernetStatusDescription[ethernetStatus.status].description}
						</div>
					</div>
				</div>
					...
					>
						...
						<div>
							...
							<input
								...
								maxlength="15"
								size="15"
								bind:value={ethernetSettings.local_ip}
								id="localIP"
								required
							/>
						...
						<div>
							...
							<input
								...
								maxlength="15"
								size="15"
								bind:value={ethernetSettings.gateway_ip}
								id="gateway"
								required
							/>
						...
						<div>
							...
							<input
								...
								maxlength="15"
								size="15"
								bind:value={ethernetSettings.subnet_mask}
								id="subnet"
								required
							/>
