 ...
 public:
  ...
  void loop1s() {
    ...
    for (int gpio_num = 0; gpio_num < SOC_GPIO_PIN_COUNT; gpio_num++) {
      ...
      task["PinInfo"] = "";
    #ifdef VOLTAGE_PIN
      if (gpio_num == VOLTAGE_PIN) task["PinInfo"] = "Voltage";
    #endif
    #ifdef CURRENT_PIN
      if (gpio_num == CURRENT_PIN) task["PinInfo"] = "Current";
    #endif
    #ifdef BATTERY_PIN
      if (gpio_num == BATTERY_PIN) task["PinInfo"] = "Battery";
    #endif
    #ifdef LED_PINS
