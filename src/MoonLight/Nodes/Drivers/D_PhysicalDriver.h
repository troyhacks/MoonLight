...
class PhysicalDriver : public DriverNode {
 public:
  ...
  void loop() override {
    ...
    #else
      show_parlio(pins, layer->layerP->lights.header.nrOfLights, layer->layerP->lights.channels, ledsDriver._brightness, layer->layerP->lights.header.channelsPerLight == 4, nb_pins, lengths[0],
  ...
  void onLayout() override {
    ...
    if (layer->layerP->pass == 1) {  // physical
      ...
      if (numPins > 0) {
        ledsDriver.initLeds(layer->layerP->lights.channels, pinConfig, numPins, layer->layerP->lights.header.channelsPerLight, layer->layerP->lights.header.offsetRed,
                            layer->layerP->lights.header.offsetGreen, layer->layerP->lights.header.offsetBlue, layer->layerP->lights.header.offsetWhite);  // 102 is GRB
